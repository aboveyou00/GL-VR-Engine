...
namespace GlEngine
{
    namespace Util
    {
        ...
        const char *const createLogFileName(const char *const path, const char *const filename) noexcept
        {
            static thread_local char buff[256];
            if (!createLogFileName(buff, sizeof(buff), path, filename)) return nullptr;
            return buff;
        }
        bool createLogFileName(char *buff, int buffSize, const char *const path, const char *const filename) noexcept
        {
