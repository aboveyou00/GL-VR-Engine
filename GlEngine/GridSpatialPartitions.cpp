...
namespace GlEngine
{
    ...
    std::string GridSpatialPartitions::computeRaycastSource = R"raw(

#version 430 core

layout(loaction = 0) uniform vec3 ray_origin;
layout(location = 1) uniform vec3 ray_direction;

layout(location = 2) uniform uint triangle_count;
layout(std430, binding = 3) buffer triangles
{
    vec3 points[];
};

float rayTriangleIntersection(vec3 orig, vec3 dir, vec3 vertices[3])
{
